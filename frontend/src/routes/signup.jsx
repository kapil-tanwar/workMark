        ...
        <div className="relative z-10 w-full max-w-[440px] space-y-6">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">I am signing up as</label>
              <div className="grid grid-cols-2 gap-1.5 p-1.5 rounded-xl border bg-muted/30 border-border">
                {[{ v: "employee", l: "Employee" }, { v: "admin", l: "Admin / HR" }].map(({ v, l }) => (
                  <button key={v} type="button" onClick={() => setRole(v)}
                    className={`py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
                      role === v ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:bg-muted"
                    }`}>
                    {l}
                  </button>
                ))}
